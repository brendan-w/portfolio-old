...
function align(withBorders)
{
	...
	for(var i = 0; i < $projects.length; i++)
	{
		...
		$currentProject.offset({top:$contentDiv.offset().top + contentGap + ($currentProject.outerHeight(true) * i),
...
function switchMode()
{
	if(!wide)
	{
		...
		for(var i = 0; i < $images.length; i++){$images[i].attr({height:h, width:w});}
	}
	else
	{
		for(var i = 0; i < $images.length; i++){$images[i].attr({height:200, width:800});}
	}
}
...
function mOver(e)
{
	//e.stopPropagation(); //isn't having the predicted effect, can't figure out why (solved with CSS pointer-event:none)
	var $target = $(e.target);
